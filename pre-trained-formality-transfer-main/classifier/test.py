...
def main():
    ...
    test_src, test_tgt = [], []
    with open('/content/drive/MyDrive/SEM2/685/Project/pre-trained-formality-transfer-main/GYAFC_Corpus/Family_Relationships/test/informal'.format(opt.dataset, opt.order),'r') as f:
        for line in f.readlines():
            test_src.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('/content/drive/MyDrive/SEM2/685/Project/pre-trained-formality-transfer-main/GYAFC_Corpus/Family_Relationships/test/formal'.format(opt.dataset, opt.order),'r') as f:
       for line in f.readlines():
    ...
    model.load_state_dict(torch.load('/content/drive/MyDrive/SEM2/685/Project/pre-trained-formality-transfer-main/checkpoints/bart_fr_0.0.0_0.chkpt'.format(
        opt.dataset)))
